...
def daily_ai_papers_tool(query:str, top_k: int = 5):
    ...
    if response.status_code != 200:
        ...
        last_updated = page_content.split("Last%20Updated-")[-1][:12].replace("--", "-")    # 获取最后更新日期
        df = pd.read_html(f"https://gabrielchua.me/daily-ai-papers/#papers-for-{last_updated}")[1]  #解析网页中所有 <table>，并选择第二个表格（论文数据）
        df = df.head(top_k)


        result = {
            "date": last_updated,
            "papers": df.to_dict(orient='records'),# 将 DataFrame 转换为字典列表
